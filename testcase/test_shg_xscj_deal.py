...
def test_shg_xscj_deal_001(shg_xscj_sb):
    ...
    if result_list[0][7] == 'S':
        ...
        set_xml(shg_xscj_path,
            {'sendDateTime': long_date, 'ordDate': local_date, 'ordTime': local_time, 'matchNo': match_no,
             'sysOrdID': sysordid,'dir':'B','bOrdCount':str(result_list[0][22]),'bOrdPrice':str(round(result_list[0][23],3)),'sOrdCount':'0','sOrdPrice':'0',
             'trader':result_list[0][14],'obTrader':result_list[0][9]})
    elif result_list[0][7] == 'B':
        set_xml(shg_xscj_path,
            {'sendDateTime': long_date, 'ordDate': local_date, 'ordTime': local_time, 'matchNo': match_no,
             'sysOrdID': sysordid,'dir':'S','bOrdCount':'0','bOrdPrice':'0','sOrdCount':str(result_list[0][20]),'sOrdPrice':str(round(result_list[0][21],3)),
             'trader':result_list[0][14],'obTrader':result_list[0][9]})
    else:
        print('输入的方向错误，请检查！！！')
    ...
    with allure.step('数据库断言：'):
        ...
        result_list = tools.oracle_link(sql)
        with allure.step('预期结果：ord_status=7'):
